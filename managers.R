...
#find the sum of missing values in age attribute
sum(is.na(managers_data$Age))

#below code give same output but logic is different
sum(!complete.cases(managers_data$Age))

#find the mean of missing values in age attribute
...
output_data <- subset(managers_data, Age > 35 | Age < 24, select = c("Gender" , "Age"))
output_data
